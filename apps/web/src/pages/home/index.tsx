...
export function HomePage() {
	...
	return (
		<Container size={420} my={40} pt={40} pos="relative">
			<Modal opened={isWaiting} onClose={stopWaiting} size={"xl"} title="Invite link created">
				<Paper withBorder shadow="md" p={30} mb={30} radius="md">
					<Flex direction={"row"} align={"center"}>
						<Image radius="md" src={"/chessboard.png"} height={60} mr={20} />
						<div>
							<Text>Time: <b>{form.values.time}' + {form.values.increment}"</b></Text>
							<Text>Side: <b>{form.values.playAs}</b></Text>
						</div>
					</Flex>
				</Paper>
				<Text>Hand this link to a friend to play with them!</Text>
				<Flex direction={"row"}>
					<TextInput readOnly value={inviteLink} mr={5} style={{ flexGrow: 1 }} />
			...
			<Paper withBorder shadow="md" p={30} mt={30} radius="md">
				...
				<TextInput label="Invite Link" mt="md" onChange={(e) => setInviteLinkToConsume(e.currentTarget.value)} />
				<Button fullWidth mt="md" disabled={!inviteLinkToConsume} onClick={consumeInviteLink}>
					Join Game
				</Button>
			</Paper>
		</Container>
